...
Fig. \ref{algo1_libc.so.6_bar} shows a speed comparison for libc.so.6, and is broken into separate graphs based on the allocation size.
This figure clearly shows the progressive time increases that both calloc and initialized stack allocations go through as the allocation size increases.
It should also be noted that malloc and uninitialized stack allocations have relatively fixed performance costs.
...
Fig. \ref{algo1_stack_malloc_hist} shows that for Algorithm \ref{light_usage_algorithm} has consistent performance in stack allocation.
It also shows the difference in performance of malloc across allocation libraries.
For most allocation libraries, the difference in performance between stack and malloc allocation is minimal.
...
\todo explain why loop unwinding is giving initialized stack an advantage over calloc in \ref{algo1_init_calloc_hist}.
...
\todo Fig. \ref{algo1_complete_threaded_hist}

\begin{figure}[tbh!]
  \centering
  \includegraphics[width=\columnwidth]{graphs/light_threaded_hist.png}
  \caption{ \todo }
  \label{algo1_complete_threaded_hist}
\end{figure}
